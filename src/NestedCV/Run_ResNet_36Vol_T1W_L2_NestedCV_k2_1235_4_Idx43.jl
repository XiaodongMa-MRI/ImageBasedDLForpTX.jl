...
Sall = size(Xall);
Xtrain = reshape(Xall[:,:,:,:,[1,2,3,5]],Sall[1],Sall[2],Sall[3],Sall[4]*4);
Ytrain = reshape(Yall[:,:,:,:,[1,2,3,5]],Sall[1],Sall[2],1,Sall[4]*4);
...
for epoch in 1:Nepochs
  ...
  model = cpu(r);
  BSON.@save string("FromMSI/NestedCV/Results/model_NCV_k2_epoch",string(epoch),".bson") model
  #CUDA.unsafe_free!(u)
  #u = gpu(uc)
